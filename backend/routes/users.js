...
const _Plant = require("../models/Plant")
...
router.route("/:id").put(checkIfAuthenticated, async (req, res) => {
    try {
        ...
        // Build explicit $set whitelist — never touch password, email, is_admin, tokens
        const $set = {}
        ...
        if (vehicle) {
            ...
            if (reg !== null) {
                if (!reg)
                    return res.status(400).json({ code: "VALIDATION", message: "Registration number cannot be empty" })
        ...
        if (!existing.vehicle && Object.keys($set).some((k) => k.startsWith("vehicle."))) {
            ...
            if (missing.length > 0) {
                return res.status(400).json({
                    code: "VALIDATION",
                    message:
                        "Vehicle registration, make and model are all required when setting vehicle for the first time"
                })
            }
        }
